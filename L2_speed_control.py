...
ki_left = 0.0                                           # integral term
kd_left = 0.0                                            # derivative term

kp_right = 0.04                                         # proportional term
ki_right = 0.0                                           # integral term
kd_right = 0.0                                            # derivative term  
                                           # derivative term
...
# Modify the driveClosedLoop function to effectively stop both motors
def driveClosedLoop(pdt, pdc, de_dt):               # this function runs motors for closed loop PID control
    global u_integral
    e = np.array(pdt - pdc)                                 # compute error
    ...
    # If the input command is a stop command, set the motor speeds to zero
    if np.array_equal(pdt, np.array([0, 0])):
        m.sendLeft(0)
        m.sendRight(0)
        return
