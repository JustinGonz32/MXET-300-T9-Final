...
def draw_star(scale_factor):
    ...
    execute_motions_closed_loop(motions)
...
# Function to execute motions in a non-blocking manner
def execute_motions_closed_loop(motions):
    for count, motion in enumerate(motions):
        print("Motion: ", count+1, "\t Chassis Forward Velocity (m/s): {:.2f} \t Chassis Angular Velocity (rad/s): {:.2f} \t Duration (sec): {:.2f}".format(motion[0], motion[1], motion[2]))
        
        # Create a thread to execute the motion
        motion_thread = threading.Thread(target=execute_motion, args=(motion,))
        motion_thread.start()
        
        # Sleep for a short duration to allow the thread to start
        time.sleep(0.1)

# Function to execute a single motion
def execute_motion(motion):
    wheel_speeds = ik.getPdTargets(motion[:2])
    pdCurrents = kin.getPdCurrent()                  
    sc.driveClosedLoop(wheel_speeds, pdCurrents, de_dt)  # Use driveClosedLoop for closed-loop control                           
    #log.tmpFile(motion[1],"fVel") 
    #log.tmpFile(motion[2],"aVel") 
    time.sleep(motion[2])  # Sleep for the duration of the motion
